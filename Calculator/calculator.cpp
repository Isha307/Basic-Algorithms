...
void add()
{
     ...
     {
          cin >> number;
          if(INT_MAX - sum < number)					// checking for overflow
	     {
               cout << "\n Integer Overflow";
               cin.clear();
               cin.ignore(INT_MAX, '\n');			 // clearing the input stream buffer
               return;
	     }
          sum = sum + number;
     }
...
void sqr()
{
     ...
     if(abs(num1) > UINT_MAX/abs(num1))			// checking for overflow
     {
          cout << "\n Integer Overflow";
          cin.clear();
          cin.ignore(INT_MAX, '\n');
...
void srt()
{
     ...
     if(num1 < 0)								// checking for neative numbers
     {
          cout << "\nNot posible to find square root!!";
          return;
     }
     q = sqrt(num1);
